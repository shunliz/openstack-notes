# 数据包从物理网卡进入[虚拟机](https://so.csdn.net/so/search?q=虚拟机&spm=1001.2101.3001.7020)的流程![](/assets/linux-network-receive-packet.png)
...
**NOTE**：由上述描述可知，开启 RPS 后，下半步软中断的数据包可以直接挂到其它 vCPU[队列](https://so.csdn.net/so/search?q=队列&spm=1001.2101.3001.7020)上，这样就能减少 vCPU2 的压力，vCPU2 就能处理更大的流量。可见，RPS 适合单网卡队列，多 vCPU 的使用场景。  
